...
local PARRY_PRESETS = { Safety={Distance=12,Debounce=0.15,Face=0.5,RequireFacing=true}, Aggressive={Distance=20,Debounce=0.05,Face=-1,RequireFacing=false} }
...
local ParryCircle = nil; local ParryCircleInner = nil
...
local FakePerks = { Flowstate={Enabled=false,Duration=3,SpeedBoost=20}, SnakeStep={Enabled=false,SpeedBoost=90}, QuickRecovery={Enabled=false}, LastVault=0, LastCrouchState=false }
...
local MoonwalkConnection = nil; local MoonwalkHeartbeat = nil; local MoonwalkButton = nil; local ParryActive = false
...
local function findRemote(path) local cur=ReplicatedStorage; for segment in string.gmatch(path,"[^%.]+") do cur=cur and cur:FindFirstChild(segment); if not cur then return nil end end return cur end
...
local function GetPos(obj) if not obj then return nil end; if obj:IsA("BasePart") then return obj.Position end; if obj:IsA("Model") then local ok,pivot=pcall(function() return obj:GetPivot().Position end); if ok then return pivot end end return nil end
    ...
    pcall(function()
        ...
        if force or LastVisualState.NoFog ~= Visual.NoFog then
            ...
            if Visual.NoFog then
                ...
                Lighting.FogEnd = VisualOriginal.FogEnd; Lighting.FogStart = VisualOriginal.FogStart
...
local function applyColorCorrection()
    ...
    local shouldEnable = Visual.ColorCorrection or (Visual.Saturation and Visual.Saturation ~= 0) or (Visual.Brightness and Visual.Brightness ~= 0) or (Visual.Contrast and Visual.Contrast ~= 0)
    ...
    pcall(function()
        if Visual.NoBlood then
            for _, v in pairs(workspace:GetDescendants()) do
                ...
                if (v:IsA("Decal") or v:IsA("Texture") or v:IsA("ParticleEmitter") or v:IsA("Trail")) and (name:find("blood") or name:find("gore") or name:find("splat")) then
                    pcall(function()
...
local function applyAntiLag()
    ...
    if AntiLag.NoParticles then
        for _, v in pairs(workspace:GetDescendants()) do
            if v:IsA("ParticleEmitter") or v:IsA("Trail") or v:IsA("Smoke") or v:IsA("Fire") then pcall(function() v.Enabled = false end) end
        end
    else
        for _, v in pairs(workspace:GetDescendants()) do
            if v:IsA("ParticleEmitter") or v:IsA("Trail") or v:IsA("Smoke") or v:IsA("Fire") then pcall(function() v.Enabled = true end) end
        end
    end
...
RunService.Heartbeat:Connect(function()
    pcall(function()
        applyVisual(); toggleScreenEffects(); applyColorCorrection()
        applyCameraShake(); removeBlood()
...
local function isRedSpark(obj)
    ...
    if not (obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles")) then return false end
    local name = string.lower(obj.Name)
    if name:find("parry") or name:find("spark") or name:find("blood") or name:find("slash") or name:find("hit") or name:find("impact") then return true end
...
print("[TiarHub v17] Part 4/5 loaded.")-- ============ ESP TAB ============
...
MiscTab:CreateSection("🌐 FPS / Ping Display")
MiscTab:CreateToggle({ name = "Show FPS/Ping", currentValue = true, flag = "fps_show", callback = function(v) if FPDisplayGui then FPDisplayGui.Enabled = v end end })
MiscTab:CreateDropdown({ name = "Display Mode", options = {"Panel", "Watermark"}, currentOption = "Panel", flag = "fps_mode", callback = function(opt) FPDisplayMode = opt end })
...
VisualTab:CreateButton({ name = "🔄 Reset Color", callback = function() resetColorCorrection(); Rayfield:Notify({ title = "Color Reset", content = "Saturation, Brightness, Contrast di-reset ke 0." }) end })
...
UISettingsTab:CreateButton({ name = "🔊 Test Sound", callback = function() playClickSound(); Rayfield:Notify({ title = "Sound", content = "Sound: " .. SoundFeedback.CurrentSound }) end })
...
-- ============================================
-- TAMBAHAN v17.1 — FPS/Ping Drag + Floating MW/AIM/AK
-- ============================================

local FPDisplayGui = Instance.new("ScreenGui")
FPDisplayGui.Name = "TiarFPDisplay"
FPDisplayGui.ResetOnSpawn = false
FPDisplayGui.IgnoreGuiInset = true
FPDisplayGui.Parent = PlayerGui
local FPDisplayMode = "Panel"
local FPFrame = nil; local FPLabel = nil; local FPStroke = nil
local FPPos = UDim2.new(0.02, 0, 0.02, 0)
pcall(function()
    if isfile and isfile("TiarFP.json") then
        local d = HttpService:JSONDecode(readfile("TiarFP.json"))
        if d.Pos then FPPos = UDim2.new(d.Pos[1], d.Pos[2], d.Pos[3], d.Pos[4]) end
    end
end)
local function saveFPPos()
    pcall(function()
        if writefile then writefile("TiarFP.json", HttpService:JSONEncode({Pos = {FPPos.X.Scale, FPPos.X.Offset, FPPos.Y.Scale, FPPos.Y.Offset}})) end
    end)
end
FPFrame = Instance.new("Frame")
FPFrame.Size = UDim2.new(0, 220, 0, 34)
FPFrame.Position = FPPos
FPFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 20)
FPFrame.BackgroundTransparency = 0.2
FPFrame.BorderSizePixel = 0
FPFrame.Parent = FPDisplayGui
Instance.new("UICorner", FPFrame).CornerRadius = UDim.new(0, 10)
FPStroke = Instance.new("UIStroke")
FPStroke.Color = Color3.fromRGB(90, 120, 210); FPStroke.Thickness = 2; FPStroke.Transparency = 0.3; FPStroke.Parent = FPFrame
FPLabel = Instance.new("TextLabel")
FPLabel.Size = UDim2.new(1, 0, 1, 0); FPLabel.BackgroundTransparency = 1
FPLabel.Text = "⚡ TiarHub ⚡ | FPS: 60 | PING: 30"
FPLabel.TextColor3 = Color3.fromRGB(220, 230, 255); FPLabel.TextStrokeTransparency = 0.5
FPLabel.TextStrokeColor3 = Color3.new(0, 0, 0); FPLabel.Font = Enum.Font.GothamBold; FPLabel.TextSize = 13
FPLabel.Parent = FPFrame
local fpDragging = false; local fpDS, fpSP
FPFrame.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
        fpDragging = true; fpDS = input.Position; fpSP = FPFrame.Position
    end
end)
UserInputService.InputChanged:Connect(function(input)
    if fpDragging and (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement) then
        local delta = input.Position - fpDS
        FPFrame.Position = UDim2.new(fpSP.X.Scale, fpSP.X.Offset + delta.X, fpSP.Y.Scale, fpSP.Y.Offset + delta.Y)
    end
end)
UserInputService.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
        if fpDragging then fpDragging = false; FPPos = FPFrame.Position; saveFPPos() end
    end
end)
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            local ping = math.floor(Stats.Network.ServerStatsItem["Data Ping"]:GetValue())
            if FPLabel then FPLabel.Text = string.format("⚡ TiarHub ⚡ | FPS: %d | PING: %d", FPS, ping) end
            if FPStroke then
                if FPDisplayMode == "Watermark" then
                    local hue = (tick() * 0.1) % 1
                    FPStroke.Color = Color3.fromHSV(hue, 0.6, 1)
                else FPStroke.Color = Color3.fromRGB(90, 120, 210) end
            end
        end)
    end
end)
        ...
        if p ~= LocalPlayer and p.Character then
            ...
            if isK then
                ...
                if hum and hum.Health>0 and ap then
                    local pos, vis = cam:WorldToViewportPoint(ap.Position)
                    if vis then local d = (Vector2.new(pos.X,pos.Y)-center).Magnitude; if d<s then s=d; c=ap end end
                end
            end
        end
    end
...
RunService.RenderStepped:Connect(function()
    pcall(function()
        if AimlockKillerCircle then
            ...
            AimlockKillerCircle.Position = Vector2.new(workspace.CurrentCamera.ViewportSize.X/2, workspace.CurrentCamera.ViewportSize.Y/2)
        end
...
RunService.RenderStepped:Connect(function()
    pcall(function()
        if AutoAimFOV then
            ...
            AutoAimFOV.Radius = AutoAimSurvivor.RadiusLock
            AutoAimFOV.Position = Vector2.new(workspace.CurrentCamera.ViewportSize.X/2, workspace.CurrentCamera.ViewportSize.Y/2)
        end
        ...
        local cam = workspace.CurrentCamera
        cam.CFrame = cam.CFrame:Lerp(CFrame.new(cam.CFrame.Position, t.Position), AutoAimSurvivor.Smoothness)
    end)
end)

local FloatGui = Instance.new("ScreenGui")
FloatGui.Name = "TiarFloat_v17"; FloatGui.ResetOnSpawn = false
FloatGui.IgnoreGuiInset = true; FloatGui.Parent = PlayerGui
local FloatPos = { MW = UDim2.new(0.05,0,0.45,0), AIM = UDim2.new(0.05,0,0.55,0), AK = UDim2.new(0.05,0,0.65,0) }
pcall(function()
    if isfile and isfile("TiarFloat_v17.json") then
        local d = HttpService:JSONDecode(readfile("TiarFloat_v17.json"))
...
local function saveFloatPos()
    pcall(function()
        if writefile then
            writefile("TiarFloat_v17.json", HttpService:JSONEncode({
...
local function makeDraggableFloat(frame, which)
...
local function makeFloatBtn(name,text,pos,color,which,onClick)
    ...
    btn.AutoButtonColor=false; btn.Parent=FloatGui; btn.Visible=false
    Instance.new("UICorner", btn).CornerRadius=UDim.new(1,0)
    local stroke = Instance.new("UIStroke")
    ...
    makeDraggableFloat(btn, which)
    return btn
end
local MWButton = makeFloatBtn("MWButton","MW",FloatPos.MW,Color3.fromRGB(170,0,255),"MW",function(btn,stroke)
    Moonwalk.Enabled = not Moonwalk.Enabled
    if Moonwalk.Enabled then
...
local AIMButton = makeFloatBtn("AIMButton","AIM",FloatPos.AIM,Color3.fromRGB(255,100,100),"AIM",function(btn,stroke)
...
local AKButton = makeFloatBtn("AKButton","AK",FloatPos.AK,Color3.fromRGB(255,60,60),"AK",function(btn,stroke)
...
local FloatTab = Window:CreateTab({ name="Floating Buttons", icon=0 })
FloatTab:CreateSection("🎛️ Tombol Floating MW / AIM / AK")
FloatTab:CreateToggle({ name="Show MW Button", currentValue=false, flag="show_mw_btn", callback=function(v) if MWButton then MWButton.Visible=v end end })
FloatTab:CreateToggle({ name="Show AIM Button", currentValue=false, flag="show_aim_btn", callback=function(v) if AIMButton then AIMButton.Visible=v end end })
FloatTab:CreateToggle({ name="Show AK Button", currentValue=false, flag="show_ak_btn", callback=function(v) if AKButton then AKButton.Visible=v end end })
FloatTab:CreateSection("⚙️ Setting Aimlock Killer")
FloatTab:CreateSlider({ name="AK FOV", range={50,1000}, increment=10, suffix="px", currentValue=200, flag="ak_fov_v17", callback=function(v) AimlockKiller.FOV=v end })
FloatTab:CreateSlider({ name="AK Smoothness", range={0.05,1}, increment=0.05, currentValue=0.35, flag="ak_sm_v17", callback=function(v) AimlockKiller.Strength=v end })
FloatTab:CreateDropdown({ name="AK Aim Part", options={"Head","UpperTorso","HumanoidRootPart"}, currentOption="Head", flag="ak_part_v17", callback=function(opt) AimlockKiller.AimPart=opt end })
FloatTab:CreateSection("⚙️ Setting Auto Aim Survivor")
FloatTab:CreateSlider({ name="AIM Radius Lock", range={50,1000}, increment=10, suffix="px", currentValue=250, flag="aal_r_v17", callback=function(v) AutoAimSurvivor.RadiusLock=v end })
FloatTab:CreateSlider({ name="AIM Smoothness", range={0.05,1}, increment=0.05, currentValue=0.4, flag="aal_sm_v17", callback=function(v) AutoAimSurvivor.Smoothness=v end })
FloatTab:CreateToggle({ name="AIM Show Radius Circle", currentValue=true, flag="aal_show_v17", callback=function(v) AutoAimSurvivor.ShowRadius=v end })

task.wait(1)
Rayfield:Notify({ title="⚡ TiarHub ⚡", content="Script loaded! v17.1 - Semua fitur siap.", duration=8 })
task.wait(2)
Rayfield:Notify({ title="🆕 Tambahan Baru", content="FPS/Ping Drag + Tombol MW/AIM/AK di tab 'Floating Buttons'", duration=8 })

print("============================================")
print("  ⚡ TIARHUB v17.1 - ALL LOADED ⚡")
print("  ==========================================")
print("  [✓] Semua fitur v17")
print("  [✓] FPS/Ping Drag-able (NEW)")
print("  [✓] Tombol Floating MW/AIM/AK (NEW)")
print("  [✓] Auto Aim Survivor + Aimlock Killer (NEW)")
print("  ==========================================")
print("  🎮 Violence District | by Tiar (v17.1)")
print("============================================")
